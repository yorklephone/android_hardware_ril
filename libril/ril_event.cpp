...
void ril_query_network_timer_remove(RIL_Token t)
{
    struct ril_event * tev = timer_list.next;
    struct ril_event * next;

    LOGE("ril_query_network_timer_remove");
    MUTEX_ACQUIRE();
    while (tev != &timer_list)
    {
        if (((UserCallbackInfo *)(tev->param))->userParam == t)
        {
            LOGE("ril_query_network_timer_remove,find token");
            removeFromList(tev);
            break;
        }
        next = tev->next;
...
void ril_query_forward_timer_remove(RIL_Token t)
{
    struct ril_event * tev = timer_list.next;
    struct ril_event * next;

    LOGE("ril_query_forward_timer_remove");
    MUTEX_ACQUIRE();
    while (tev != &timer_list)
    {
        if (((UserCallbackInfo *)(tev->param))->userParam == t)
        {
            LOGE("ril_query_forward_timer_remove,find token");
            removeFromList(tev);
            break;
        }
        next = tev->next;
