...
#ifdef TARGET_IS_LEPHONE
static void dispatchKlsa(Parcel& p, RequestInfo *pRI);
#endif
...
#ifdef TARGET_IS_LEPHONE
static int responseAllPinStatusCnf(Parcel &p, void *response, size_t responselen);
static int responseSimCardTypeCnf(Parcel &p, void *response, size_t responselen);
#endif
...
static void dispatchKlsa(Parcel &p, RequestInfo *pRI) {
    ...
    status= p.readInt32(&t);
    value.time = (int)t;
    s_callbacks.onRequest(pRI->pCI->requestNumber, &value, sizeof(value), pRI);
    return;
...
#ifdef TARGET_IS_LEPHONE
        p.writeInt32(p_cur->GW_Ecio);
        p.writeInt32(p_cur->arfcn);
        p.writeInt32(p_cur->uarfcn);
#endif
...
static int responseAllPinStatusCnf(Parcel &p, void *response, size_t responselen){
    ...
    LOGE("responseAllPinStatusCnf");
    RIL_AllPinStatus *p_cur = ((RIL_AllPinStatus *) response);
    startResponse;

    p.writeInt32(4);
    p.writeInt32(p_cur->pin1_retry);
    p.writeInt32(p_cur->pin1_puk_retry);
    p.writeInt32(p_cur->pin2_retry);
    p.writeInt32(p_cur->pin2_puk_retry);
    closeResponse;
    return 0;
}

static int responseSimCardTypeCnf(Parcel &p, void *response, size_t responselen){
	LOGE("responseSimCardTypeCnf");
	startResponse;
	p.writeInt32(1);
	p.writeInt32(*(int *)response);
	closeResponse;
	return 0;
}
#endif
...
RIL_onRequestComplete(RIL_Token t, RIL_Errno e, void *response, size_t responselen) {
    ...
    if (pRI->pCI->requestNumber == RIL_REQUEST_QUERY_AVAILABLE_NETWORKS) {
        ril_query_network_timer_remove(t);
    }
	   if (pRI->pCI->requestNumber == RIL_REQUEST_QUERY_CALL_FORWARD_STATUS) {
		   ril_query_forward_timer_remove(t);
	   }
#endif
...
#endif
#ifdef TARGET_IS_LEPHONE
        case RIL_E_QUERY_NETWORK_ABORT:return "E_QUERY_NETWORK_ABORT";
        case RIL_E_QUERY_NETWORK_TIMEOUT:return "RIL_E_QUERY_NETWORK_TIMEOUT";
        case RIL_E_QUERY_FORWARD_TIMEOUT:return "E_QUERY_FORWARD_TIMEOUT";
#endif
...
#ifdef TARGET_IS_LEPHONE
        case RIL_REQUEST_SET_HEATBEAT_PACKET: return "SET_HEATBEAT_PACKET";
        case RIL_REQUEST_START_HEATBEAT: return "START_HEATBEAT";
        case RIL_REQUEST_STOP_HEATBEAT: return "STOP_HEATBEAT";
        case RIL_REQUEST_GET_ALL_PIN_STATUS: return "GET_ALL_PIN_STATUS";
        case RIL_REQUEST_GET_SIM_CARD_TYPE: return "GET_SIM_CARD_TYPE";
        case RIL_REQUEST_SET_MORE_SMS: return "SET_MORE_SMS";
        case RIL_REQUEST_DATA_DORMANT_TIME: return "SET_DATA_DORMANT_TIME";
#endif
...
#ifdef TARGET_IS_LEPHONE
        case RIL_UNSOL_RESPONSE_NEW_SMS_ON_UIM: return "UNSOL_RESPONSE_NEW_SMS_ON_UIM";
#endif
